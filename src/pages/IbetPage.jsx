...
import { initial_ibet_columns, ibet_columns } from "../constants/index";
import {
  Button,
  Dropdown,
  DropdownItem,
...
const IbetPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://127.0.0.1:3000/v1/ibets", {
          method: "GET",
        });
  ...
  const renderCell = useCallback(
    (datas, columnKey) => {
      ...
      switch (columnKey) {
        ...
        case "amount":
          return <h3>{datas.amount}</h3>;
        case "valid_amount":
          return <h3>{datas.valid_amount}</h3>;
        case "bet_amount":
  ...
  return (
    <div className="p-4">
      ...
      <TableBox
        ...
        initial_visible_columns={initial_ibet_columns}
        columns={ibet_columns}
